...
//! Multithreaded wrapper around TCPConnection that approximates the Unix
//! sockets API
...
private:
  ...
  //! Handle to the TCPConnection thread; owner thread calls join() in the
  //! destructor
  ...
  std::atomic_bool _abort{false}; //!< Flag used by the owner to force the
                                  //!< TCPConnection thread to shut down
...
public:
  //! Construct from the interface that the TCPConnection thread will use to
  //! read and write datagrams
...
using TCPOverIPv4OverEthernetSpongeSocket =
    TCPSpongeSocket<TCPOverIPv4OverEthernetAdapter>;

using LossyTCPOverUDPSpongeSocket =
    TCPSpongeSocket<LossyTCPOverUDPSocketAdapter>;
using LossyTCPOverIPv4SpongeSocket =
    TCPSpongeSocket<LossyTCPOverIPv4OverTunFdAdapter>;
...
//!
//! The other, the "TCPConnection" thread, takes care of the back-end tasks that
//! the kernel would perform for a TCPSocket: reading and parsing datagrams from
//! the wire, filtering out segments unrelated to the connection, etc.
//!
//! There are a few notable differences between the TCPSpongeSocket and
...
//! - if TCPSpongeSocket is destructed while a TCP connection is open, the
//! connection is
//!   immediately terminated with a RST (call `wait_until_closed` to avoid this)

//! Helper class that makes a TCPOverIPv4SpongeSocket behave more like a
//! (kernel) TCPSocket
...
//! Helper class that makes a TCPOverIPv4overEthernetSpongeSocket behave more
//! like a (kernel) TCPSocket
...
public:
  ...
  //! those IP datagrams in Ethernet frames sent to the Ethernet address of the
  //! next hop.
  FullStackSocket();
  void connect(const Address &address);
};
