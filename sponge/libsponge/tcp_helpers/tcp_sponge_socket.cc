...
//! sockets
//! \param[in] datagram_interface is the interface for reading and writing
//! datagrams
template <typename AdaptT>
...
void TCPSpongeSocket<AdaptT>::_initialize_TCP(const TCPConfig &config) {
  ...
  _eventloop.add_rule(
      _datagram_adapter, Direction::In,
      [&] {
        ...
        // debugging output:
        if (_thread_data.eof() and _tcp.value().bytes_in_flight() == 0 and
            not _fully_acked) {
  ...
  _eventloop.add_rule(
      _thread_data, Direction::In,
      [&] {
        const auto data =
            _thread_data.read(_tcp->remaining_outbound_capacity());
        ...
        if (_thread_data.eof()) {
          ...
          cerr << "DEBUG: Outbound stream to "
               ...
               << (_tcp.value().bytes_in_flight() == 1 ? "" : "s")
               << " still in flight).\n";
        }
      },
      [&] {
  ...
  _eventloop.add_rule(
      ...
      [&] {
        ...
        const size_t amount_to_write =
            min(size_t(65536), inbound.buffer_size());
        ...
        if (inbound.eof() or inbound.error()) {
          ...
          if (_tcp.value().state() == TCPState::State::TIME_WAIT) {
            cerr << "DEBUG: Waiting for lingering segments (e.g. "
...
//! Unix-domain sockets of specified type
//! \param[in] type is the type of AF_UNIX sockets to create (e.g.,
//! SOCK_SEQPACKET)
...
socket_pair_helper(const int type) {
  ...
  SystemCall("socketpair",
             ::socketpair(AF_UNIX, type, 0, static_cast<int *>(fds)));
...
TCPSpongeSocket<AdaptT>::TCPSpongeSocket(AdaptT &&datagram_interface)
    : TCPSpongeSocket(socket_pair_helper(SOCK_STREAM),
...
template <typename AdaptT> void TCPSpongeSocket<AdaptT>::wait_until_closed() {
  shutdown(SHUT_RDWR);
  if (_tcp_thread.joinable()) {
...
void TCPSpongeSocket<AdaptT>::connect(const TCPConfig &c_tcp,
  ...
  if (_tcp->state() != expected_state) {
    throw runtime_error("After TCPConnection::connect(), state was " +
                        _tcp->state().name() + " but expected " +
                        expected_state.name());
  }
...
template <typename AdaptT> void TCPSpongeSocket<AdaptT>::_tcp_main() {
  try {
...
FullStackSocket::FullStackSocket()
    : TCPOverIPv4OverEthernetSpongeSocket(TCPOverIPv4OverEthernetAdapter(
          TapFD("tap10"), random_private_ethernet_address(),
