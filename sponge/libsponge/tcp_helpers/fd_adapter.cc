...
//! \details This function first attempts to parse a TCP segment from the next
//! UDP payload recv()d from the socket.
//!
...
//! correct.
//!
//! If the TCP FSM is listening (i.e., TCPOverUDPSocketAdapter::_listen is
//! `true`) and the TCP segment read from the wire includes a SYN, this function
//! clears the
//! `_listen` flag and calls calls connect() on the underlying UDP socket, with
//! the result that future outgoing segments go to the sender of the SYN
//! segment.
//! \returns a std::optional<TCPSegment> that is empty if the segment was
