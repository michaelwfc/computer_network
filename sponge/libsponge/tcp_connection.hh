...
public:
  ...
  //! put each one into the payload of a lower-layer datagram (usually Internet
  //! datagrams (IP), but could also be user datagrams (UDP) or any other kind).
  std::queue<TCPSegment> &segments_out() { return _segments_out; }

  //! \brief Is the connection still alive in any way?
  //! \returns `true` if either stream is still running or if the TCPConnection
  //! is lingering after both streams have finished (e.g. to ACK retransmissions
  //! from the peer)
  bool active() const;
  //!@}
