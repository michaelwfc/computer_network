...
// Translates from {IP datagram, next hop address} to link-layer frame, and from
// link-layer frame to IP datagram
...
//! the interface
//! \param[in] ip_address IP (what ARP calls "protocol") address of the
//! interface
NetworkInterface::NetworkInterface(const EthernetAddress &ethernet_address,
  ...
  cerr << "DEBUG: Network interface has Ethernet address "
       << to_string(_ethernet_address) << " and IP address " << ip_address.ip()
       << "\n";
}
