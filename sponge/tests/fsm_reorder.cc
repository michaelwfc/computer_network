  ...
  try {
    ...
    for (unsigned rep_no = 0; rep_no < NREPS; ++rep_no) {
      ...
      TCPTestHarness test_1 =
          TCPTestHarness::in_established(cfg, tx_isn, rx_isn);
      vector<tuple<size_t, size_t>> seq_size;
      size_t datalen = 0;
      while (datalen < cfg.recv_capacity) {
        const size_t size = min(
            1 + (static_cast<size_t>(rd()) % (TCPConfig::MAX_PAYLOAD_SIZE - 1)),
      ...
      for (auto [off, sz] : seq_size) {
        test_1.send_data(rx_isn + 1 + off, tx_isn + 1, d.cbegin() + off,
                         d.cbegin() + off + sz);
        ...
        TCPSegment seg =
            test_1.expect_seg(ExpectSegment{}.with_ack(true),
      ...
      test_1.execute(ExpectData{}.with_data(d),
                     "test 1 failed: got back the wrong data");
    }
    ...
    for (unsigned rep_no = 0; rep_no < NREPS; ++rep_no) {
      ...
      while (datalen < cfg.recv_capacity) {
        ...
        if (rem == 0) {
          ...
          offs = min(min(datalen, rem),
                     1 + (static_cast<size_t>(rd()) % (rem - 1)));
        }
      ...
      test_err_if(datalen > cfg.recv_capacity,
      ...
      WrappingInt32 max_expect_ackno = rx_isn + 1;
      for (auto [off, sz] : seq_size) {
        test_2.send_data(rx_isn + 1 + off, tx_isn + 1, d.cbegin() + off,
                         d.cbegin() + off + sz);
        if (off <= min_expect_ackno.raw_value() &&
        ...
        max_expect_ackno =
            max_expect_ackno + sz; // really loose because of overlap

        TCPSegment seg =
            test_2.expect_seg(ExpectSegment{}.with_ack(true),
      ...
      test_2.execute(ExpectData{}.with_data(d),
                     "test 1 failed: got back the wrong data");
    }
