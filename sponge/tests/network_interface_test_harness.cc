...
NetworkInterfaceExpectationViolation::NetworkInterfaceExpectationViolation(
    const std::string &msg)
    : std::runtime_error(msg) {}

template <typename T>
NetworkInterfaceExpectationViolation
NetworkInterfaceExpectationViolation::property(const std::string &property_name,
                                               const T &expected,
                                               const T &actual) {
  return NetworkInterfaceExpectationViolation(
      "The NetworkInterface should have had " + property_name + " equal to " +
...
std::string NetworkInterfaceExpectation::description() const {
...
std::string NetworkInterfaceAction::description() const {
  return "description missing";
}
...
NetworkInterfaceTestHarness::NetworkInterfaceTestHarness(
    const std::string &test_name, const EthernetAddress &ethernet_address,
...
void NetworkInterfaceTestHarness::execute(
    const NetworkInterfaceTestStep &step) {
  try {
...
string ReceiveFrame::description() const {
...
void ReceiveFrame::execute(NetworkInterface &interface) const {
  ...
  if ((not result.has_value()) and (not expected.has_value())) {
    ...
    throw NetworkInterfaceExpectationViolation(
        "an arriving Ethernet frame was expected to be passed up the stack as "
        "an Internet datagram, but wasn't");
  } else if (result.value().serialize().concatenate() !=
             expected.value().serialize().concatenate()) {
    throw NetworkInterfaceExpectationViolation(
        string("NetworkInterface::recv_frame() produced a different Internet "
               "datagram than was expected: ") +
        "actual={" + result.value().header().summary() + "}");
  }
}

string ExpectFrame::description() const {
...
void ExpectFrame::execute(NetworkInterface &interface) const {
  ...
  if (interface.frames_out().front().serialize().concatenate() !=
    ...
    throw NetworkInterfaceExpectationViolation(
        "NetworkInterface sent a different Ethernet frame than was expected: "
        "actual={" +
