...
int main(int argc, char **argv) {
  try {
    ...
    for (unsigned i = 0; i < NREPS; ++i) {
      ...
      test_header[12] = 0x40;
      {
        const auto new_cksum =
            inet_cksum(test_header.data(), test_header.size());
        test_header[16] = new_cksum >> 8;
        test_header[17] = new_cksum & 0xff;
        NetParser p{string(test_header.begin(), test_header.end())};
        if (const auto res = test_1.parse(p);
            res != ParseResult::HeaderTooShort) {
          throw runtime_error(
      ...
      /* // checksum is taken over whole segment, so only TCPSegment parser
         checks the checksum
          {
              NetParser p{string(test_header.begin(), test_header.end())};
              if (const auto res = test_1.parse(p); res !=
         ParseResult::BadChecksum) { throw runtime_error("bad parse: got wrong
         error for incorrect checksum: " + as_string(res));
              }
          }
      */
      ...
      test_header[17] = checksum & 0xff;
      {
        NetParser p{string(test_header.begin(), test_header.end())};
        if (const auto res = test_1.parse(p);
            res != ParseResult::PacketTooShort) {
          throw runtime_error(
    ...
    // now process some segments off the wire for correctness of parser and
    // unparser
    if (argc < 2) {
    ...
    while ((pkt = pcap_next(pcap, &hdr)) != nullptr) {
      ...
      // parse succeeded. Create a new segment and rebuild the header by
      // unparsing.
      cout << dec;
      ...
      if (const auto res =
              tcp_seg_copy2.parse(tcp_seg_copy.serialize().concatenate());
          res != ParseResult::NoError) {
        auto tcp_parse_result = as_string(res);
        cout << "ERROR got parse failure " << tcp_parse_result
             << " for this segment:\n";
        cout << endl << "original:\n";
        hexdump(tcp_seg_data, tcp_seg_len);
        ok = false;
