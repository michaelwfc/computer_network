...
struct BytesAvailable : public ReassemblerExpectation {
  ...
  std::string description() const {
    ...
    ss << "stream_out().buffer_size() returned " << _bytes.size()
       << ", and stream_out().read(" << _bytes.size()
  ...
  void execute(StreamReassembler &reassembler) const {
    if (reassembler.stream_out().buffer_size() != _bytes.size()) {
      ...
      ss << "The reassembler was expected to have `" << _bytes.size()
         << "` bytes available, but there were `"
...
struct UnassembledBytes : public ReassemblerExpectation {
  ...
  void execute(StreamReassembler &reassembler) const {
    if (reassembler.unassembled_bytes() != _bytes) {
      std::ostringstream ss;
      ss << "The reassembler was expected to have `" << _bytes
         << "` bytes not assembled, but there were `"
...
struct SubmitSegment : public ReassemblerAction {
  ...
  std::string description() const {
    std::ostringstream ss;
    ss << "substring submitted with data \"" << _data << "\", index `" << _index
...
public:
  ReassemblerTestHarness(const size_t capacity)
      : reassembler(capacity), steps_executed() {
    steps_executed.emplace_back(
        "Initialized (capacity = " + std::to_string(capacity) + ")");
  }
