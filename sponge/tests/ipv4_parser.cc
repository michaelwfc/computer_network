...
int main(int argc, char **argv) {
  try {
    ...
    for (unsigned i = 0; i < NREPS; ++i) {
      ...
      if (uint16_t tval = (test_header[10] << 8) | test_header[11];
          test_hdr.cksum != tval) {
      ...
      test_header[0] = 0x55;
      {
        ...
        if (auto ret = test_hdr.parse(p); ret != ParseResult::WrongIPVersion) {
          throw runtime_error(
              "Parse error: failed to detect wrong IP version.");
        }
      }
      ...
      test_header[15] = (cksum & 0xff) + max(int(rd()), 1);
      {
        ...
        if (auto ret = test_hdr.parse(p); ret != ParseResult::BadChecksum) {
          throw runtime_error(
              "Parse error: failed to detect incorrect checksum");
        }
      }
      ...
      test_header.resize(20);
      {
        ...
        if (auto ret = test_hdr.parse(p); ret != ParseResult::PacketTooShort) {
          throw runtime_error(
              "Parse error: failed to detect packet too short to parse");
        }
      }
    ...
    if (pcap == nullptr) {
      cout << "ERROR opening " << argv[1] << ": " << static_cast<char *>(errbuf)
           << endl;
      return EXIT_FAILURE;
    }
    ...
    while ((pkt = pcap_next(pcap, &hdr)) != nullptr) {
      ...
      TCPSegment tcp_seg;
      if (auto res = tcp_seg.parse(ip_dgram.payload(),
      ...
      if (expect_fail) {
        cout << "ERROR: expected parse failure but got success. Something is "
                "wrong.\n";
        show_ethernet_frame(pkt, hdr);
        hexdump(pkt + 14, hdr.caplen - 14);
        ok = false;
      ...
      // parse succeeded. Create a new packet and rebuild the header by
      // unparsing.
      cout << dec;
      ...
      if (!compare_ip_headers_nolen(ip_dgram.header(),
                                    ip_dgram_copy.header())) {
        cout << "ERROR: after unparsing, IP headers (other than length) don't "
                "match.\n";
      }
      if (!compare_tcp_headers_nolen(tcp_seg.header(), tcp_seg_copy.header())) {
        cout << "ERROR: after unparsing, TCP headers (other than length) don't "
                "match.\n";
      }
      ...
      concat.append(
          tcp_seg_copy.serialize(ip_dgram_copy.header().pseudo_cksum())
              .concatenate());

      if (auto res = ip_dgram_copy2.parse(string(concat));
          res != ParseResult::NoError) {
        auto ip_parse_result = as_string(res);
      ...
      if (auto res = tcp_seg_copy2.parse(
              ip_dgram_copy2.payload(), ip_dgram_copy2.header().pseudo_cksum());
          res != ParseResult::NoError) {
        auto tcp_parse_result = as_string(res);
      ...
      if (!compare_ip_headers_nolen(ip_dgram.header(),
                                    ip_dgram_copy2.header())) {
        cout << "ERROR: after re-parsing, IP headers don't match (0<->2).\n";
        ok = false;
        continue;
      }
      if (!compare_ip_headers(ip_dgram_copy.header(),
                              ip_dgram_copy2.header())) {
        cout << "ERROR: after re-parsing, IP headers don't match (1<->2).\n";
        ok = false;
        continue;
      }
      if (!compare_tcp_headers_nolen(tcp_seg.header(),
                                     tcp_seg_copy2.header())) {
      ...
      if (tcp_seg_copy.payload().str() != tcp_seg_copy2.payload().str()) {
        ...
        hexdump(tcp_seg_copy2.payload().str().data(),
                tcp_seg_copy2.payload().str().size());
        cout << endl;
        hexdump(tcp_seg_copy.payload().str().data(),
                tcp_seg_copy.payload().str().size());
        ok = false;
        continue;
      }
    }
