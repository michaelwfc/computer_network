...
int main() {
  try {
    ...
    // test #1: START -> LISTEN -> SYN -> SYN/ACK -> ACK
    {
      ...
      /* remove requirement for corrective ACK in response to out-of-window
         seqno const WrappingInt32 syn_seqno = seg.header().seqno;
          test_1.send_ack(WrappingInt32{0}, syn_seqno + 1);
          test_1.execute(Tick(1));

          test_1.execute(
              ExpectOneSegment{}.with_no_flags().with_ack(true).with_ackno(1).with_seqno(seg.header().seqno
         + 1), "test 1 failed: wrong response to old seqno");
