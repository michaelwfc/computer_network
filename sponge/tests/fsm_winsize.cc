...
int main() {
  try {
    ...
    // test 1: listen -> established -> check advertised winsize -> check sent
    // bytes before ACK
    for (unsigned rep_no = 0; rep_no < NREPS; ++rep_no) {
      ...
      test_1.send_ack(seq_base + 1, ack_base + 1,
                      swin); // adjust send window here
      ...
      const unsigned swin_mul =
          MIN_SWIN_MUL + (rd() % (MAX_SWIN_MUL - MIN_SWIN_MUL));
      ...
      while (bytes_total < swin_mul * swin) {
        ...
        while (test_1.can_read()) {
          ...
          copy(seg2.payload().str().cbegin(), seg2.payload().str().cend(),
        ...
        // NOTE that we don't override send window here because cfg should have
        // been updated
