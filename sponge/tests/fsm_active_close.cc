...
int main() {
  try {
    ...
    // test #4: start in FIN_WAIT_1, ack, FIN, time out
    {
      ...
      test_4.execute(
          ExpectOneSegment{}.with_no_flags().with_ack(true).with_ackno(
              ack_expect));
    ...
    // test 5: start in FIN_WAIT_1, ack, FIN, FIN again, time out
    {
      ...
      // tick the timer and see what happens---a 2nd FIN in TIME_WAIT should
      // reset the wait timer! (this is an edge case of "throw it away and send
      // another ack" for out-of-window segs)
      test_5.execute(Tick(10 * cfg.rt_timeout - 10));

      test_5.execute(ExpectLingerTimer{uint64_t(10 * cfg.rt_timeout - 9)},
                     "test 5 failed: time_since_last_segment_received() should "
                     "reset after 2nd FIN");
    ...
    // test 6: start in ESTABLISHED, get FIN, get FIN re-tx, send FIN, get ACK,
    // send ACK, time out
    {
      ...
      TCPSegment seg1 =
          test_6.expect_seg(ExpectOneSegment{}.with_fin(true),
      ...
      test_6.execute(ExpectNoSegment{},
                     "test 6 failed: FIN re-tx was too fast");

      test_6.execute(Tick(2));

      TCPSegment seg2 = test_6.expect_seg(
          ExpectOneSegment{}.with_fin(true).with_seqno(seg1_hdr.seqno),
