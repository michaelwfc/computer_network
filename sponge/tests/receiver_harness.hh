...
public:
  ReceiverExpectationViolation(const std::string msg)
    ...
    if (receiver.ackno() != _ackno) {
      std::string reported =
          receiver.ackno().has_value()
              ...
              : "none";
      std::string expected = _ackno.has_value()
                                 ? std::to_string(_ackno.value().raw_value())
                                 : "none";
      throw ReceiverExpectationViolation(
          "The TCPReceiver reported ackno `" + reported +
...
struct ExpectWindow : public ReceiverExpectation {
  ...
  void execute(TCPReceiver &receiver) const {
    if (receiver.window_size() != _window) {
      ...
      throw ReceiverExpectationViolation(
          "The TCPReceiver reported window `" + reported +
          "`, but it was expected to be `" + expected + "`");
    }
  }
};
...
struct ExpectTotalAssembledBytes : public ReceiverExpectation {
  ...
  void execute(TCPReceiver &receiver) const {
    if (receiver.stream_out().bytes_written() != _n_bytes) {
      std::ostringstream ss;
      ss << "The TCPReceiver stream reported `"
         << receiver.stream_out().bytes_written()
...
struct ExpectEof : public ReceiverExpectation {
  ExpectEof() {}
  std::string description() const {
    return "receiver.stream_out().eof() == true";
  }

  void execute(TCPReceiver &receiver) const {
    if (not receiver.stream_out().eof()) {
      throw ReceiverExpectationViolation(
          "The TCPReceiver stream reported eof() == false, but was expected to "
          "be true");
    }
  }
};
...
struct ExpectInputNotEnded : public ReceiverExpectation {
  ExpectInputNotEnded() {}
  std::string description() const {
    return "receiver.stream_out().input_ended() == false";
  }

  void execute(TCPReceiver &receiver) const {
    if (receiver.stream_out().input_ended()) {
      throw ReceiverExpectationViolation(
          "The TCPReceiver stream reported input_ended() == true, but was "
  ...
  void execute(TCPReceiver &receiver) const {
    ...
    if (stream.buffer_size() != _bytes.size()) {
      ...
      ss << "The TCPReceiver reported `" << stream.buffer_size()
         << "` bytes available, but there were expected to be `"
         << _bytes.size() << "` bytes available";
      throw ReceiverExpectationViolation(ss.str());
    }
    std::string bytes = stream.read(_bytes.size());
    if (not std::equal(bytes.begin(), bytes.end(), _bytes.begin(),
                       _bytes.end())) {
      std::ostringstream ss;
      ss << "the TCPReceiver assembled \"" << bytes
         << "\", but was expected to assemble \"" << _bytes << "\".";
      throw ReceiverExpectationViolation(ss.str());
    }
  }
};
...
struct SegmentArrives : public ReceiverAction {
  ...
  SegmentArrives &with_ack(uint32_t ackno_) {
  ...
  void execute(TCPReceiver &receiver) const override {
    ...
    if (result.has_value() and result.value() != res) {
      throw ReceiverExpectationViolation(
          "TCPReceiver::segment_received() reported `" + result_name(res) +
          "` in response to `" + o.str() +
          "`, but it was expected to report `" + result_name(result.value()) +
          "`");
    }
  }
};
...
public:
  TCPReceiverTestHarness(size_t capacity)
      : receiver(capacity), steps_executed() {
    std::ostringstream ss;
    ss << "Initialized with ("
       << "capacity=" << capacity << ")";
