...
public:
  ...
  virtual TCPSegment expect_seg(TCPTestHarness &) const {
...
public:
  ...
  template <typename T>
  static TCPPropertyViolation make(const std::string &property_name,
  ...
  template <typename T>
  static TCPPropertyViolation make_not(const std::string &property_name,
                                       const T &expected_non_value) {
    std::stringstream ss{};
    ss << "The TCP has `" << property_name << " = " << expected_non_value
       << "`, but " << property_name << " was expected to **not** be `"
       << expected_non_value << "`";
    return TCPPropertyViolation{ss.str()};
  }
};
...
public:
  ...
  violated_field(const std::string &field_name, const T &expected_value,
                 const T &actual_value) {
    std::stringstream ss{};
    ss << "The TCP produced a segment with `" << field_name << " = "
...
public:
  StateExpectationViolation(const std::string &msg)
      : TCPExpectationViolation(msg) {}
  StateExpectationViolation(const TCPState &expected_state,
                            const TCPState &actual_state)
