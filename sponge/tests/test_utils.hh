...
inline void show_ethernet_frame(const uint8_t *pkt,
  ...
  std::cout << "    ethertype: " << std::setw(2) << +pkt[12] << std::setw(2)
            << +pkt[13] << '\n';
  std::cout << std::dec << "length: " << hdr.len
            << "    captured: " << hdr.caplen << '\n';

  std::cout.flags(flags);
}

inline bool compare_tcp_headers_nolen(const TCPHeader &h1,
                                      const TCPHeader &h2) {
  return h1.sport == h2.sport && h1.dport == h2.dport && h1.seqno == h2.seqno &&
         h1.ackno == h2.ackno && h1.urg == h2.urg && h1.ack == h2.ack &&
         h1.psh == h2.psh && h1.rst == h2.rst && h1.syn == h2.syn &&
