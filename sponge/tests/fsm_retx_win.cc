  ...
  try {
    ...
    // multiple segments with intervening ack
    {
      ...
      test_2.send_ack(tx_ackno,
                      tx_ackno +
                          4); // make sure RTO timer restarts on successful ACK
    ...
    // multiple segments without intervening ack
    {
      ...
      test_3.execute(ExpectSegmentAvailable{},
                     "test 3 failed: cannot read after write()s");
      ...
      test_3.execute(ExpectNoSegment{},
                     "test 3 failed: re-tx of 2nd seg too fast");
    ...
    auto backoff_test = [&](const unsigned int num_backoffs) {
      ...
      test_4.execute(ExpectSegmentAvailable{},
                     "test 4 failed: cannot read after write()s");
      ...
      for (unsigned i = 1; i < num_backoffs; ++i) {
        test_4.execute(Tick((cfg.rt_timeout << i) -
                            i)); // exponentially increasing delay length
        test_4.execute(ExpectNoSegment{},
                       "test 4 failed: re-tx too fast after timeout");
      ...
      test_4.execute(ExpectNoSegment{}, "test 4 failed: re-tx of 2nd seg after "
                                        "ack for 1st seg too fast after " +
                                            to_string(num_backoffs) +
                                            " backoffs");
