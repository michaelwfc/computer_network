...
int main() {
  try {
    ...
    // test #2: start in CLOSE_WAIT, close(), throw away first FIN, ack re-tx
    // FIN
    {
      ...
      test_2.execute(ExpectNoSegment{},
                     "test 2 failed: FIN re-tx was too fast");

      test_2.execute(Tick(2));

      TCPSegment seg2 = test_2.expect_seg(
          ExpectOneSegment{}.with_fin(true).with_seqno(seg1.header().seqno),
