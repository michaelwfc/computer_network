  ...
  try {
    ...
    for (unsigned rep_no = 0; rep_no < NREPS; ++rep_no) {
      ...
      while (sendoff < d.size()) {
        const size_t len =
            min(d.size() - sendoff, static_cast<size_t>(rd()) % 8192);
        if (len == 0) {
          continue;
        }
        ...
        for (size_t i = 0; i < n_segments; ++i) {
          size_t expected_size =
              min(bytes_remaining, TCPConfig::MAX_PAYLOAD_SIZE);
          auto seg = test_1.expect_seg(
              ExpectSegment{}.with_payload_size(expected_size));
        ...
        for (size_t i = 0; i < n_segments; ++i) {
          auto seg = test_1.expect_seg(
              ExpectSegment{}.with_ack(true).with_payload_size(0));
          test_1.execute(SendSegment{move(seg)});
          test_1.execute(Tick(1));
        }
      ...
      test_1.execute(ExpectData{}.with_data(d),
                     "test 1 falied: got back the wrong data");
    }
