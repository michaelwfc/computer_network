...
EthernetFrame make_frame(const EthernetAddress &src, const EthernetAddress &dst,
                         const uint16_t type, const BufferList payload) {
  EthernetFrame frame;
  ...
  try {
    {
      ...
      test.execute(ExpectFrame{
          make_frame(local_eth, ETHERNET_BROADCAST, EthernetHeader::TYPE_ARP,
                     make_arp(ARPMessage::OPCODE_REQUEST, local_eth, "4.3.2.1",
                              {}, "192.168.0.1")
                         .serialize())});
      ...
      test.execute(ReceiveFrame{
          make_frame(target_eth, local_eth, EthernetHeader::TYPE_ARP,
                     make_arp(ARPMessage::OPCODE_REPLY, target_eth,
...

    {
      ...
      test.execute(ReceiveFrame{
          make_frame(remote_eth, ETHERNET_BROADCAST, EthernetHeader::TYPE_ARP,
                     make_arp(ARPMessage::OPCODE_REQUEST, remote_eth,
                              "10.0.1.1", {}, "7.7.7.7")
                         .serialize()),
          {}});
      test.execute(ExpectNoFrame{});
      ...
      test.execute(ExpectFrame{
          make_frame(local_eth, remote_eth, EthernetHeader::TYPE_ARP,
                     make_arp(ARPMessage::OPCODE_REPLY, local_eth, "5.5.5.5",
                              remote_eth, "10.0.1.1")
                         .serialize())});
      test.execute(ExpectNoFrame{});
    }
...

    {
      ...
      test.execute(ExpectFrame{
          make_frame(local_eth, ETHERNET_BROADCAST, EthernetHeader::TYPE_ARP,
                     make_arp(ARPMessage::OPCODE_REQUEST, local_eth, "1.2.3.4",
                              {}, "10.0.0.1")
                         .serialize())});
      test.execute(ExpectNoFrame{});
...

    {
      ...
      test.execute(ReceiveFrame{
          make_frame(new_target_eth, local_eth, EthernetHeader::TYPE_ARP,
                     make_arp(ARPMessage::OPCODE_REPLY, new_target_eth,
                              "192.168.0.1", local_eth, "4.3.2.1")
                         .serialize()),
          {}});
...

    {
      ...
      test.execute(ReceiveFrame{
          make_frame(remote_eth1, ETHERNET_BROADCAST, EthernetHeader::TYPE_ARP,
                     make_arp(ARPMessage::OPCODE_REQUEST, remote_eth1,
                              "10.0.0.5", {}, "10.0.0.1")
                         .serialize()),
          {}});
      test.execute(ExpectFrame{
          make_frame(local_eth, remote_eth1, EthernetHeader::TYPE_ARP,
                     make_arp(ARPMessage::OPCODE_REPLY, local_eth, "10.0.0.1",
      ...
      test.execute(ReceiveFrame{
          make_frame(remote_eth2, ETHERNET_BROADCAST, EthernetHeader::TYPE_ARP,
                     make_arp(ARPMessage::OPCODE_REQUEST, remote_eth2,
                              "10.0.0.19", {}, "10.0.0.1")
                         .serialize()),
          {}});
      test.execute(ExpectFrame{
          make_frame(local_eth, remote_eth2, EthernetHeader::TYPE_ARP,
                     make_arp(ARPMessage::OPCODE_REPLY, local_eth, "10.0.0.1",
      ...
      test.execute(ExpectFrame{
          make_frame(local_eth, ETHERNET_BROADCAST, EthernetHeader::TYPE_ARP,
                     make_arp(ARPMessage::OPCODE_REQUEST, local_eth, "10.0.0.1",
                              {}, "10.0.0.5")
                         .serialize())});
      test.execute(ExpectNoFrame{});
    }
