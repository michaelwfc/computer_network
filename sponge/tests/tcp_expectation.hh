  ...
  void execute(TCPTestHarness &harness) const {
    ...
    if (data.has_value()) {
      if (actual_data.size() != data->size()) {
        ...
        msg << actual_data.size() << " bytes, but should have produced "
            << data->size() << " bytes";
        throw TCPExpectationViolation(msg.str());
      }
      if (not std::equal(actual_data.begin(), actual_data.end(),
                         data->begin())) {
        std::ostringstream msg{"The TCP produced data "};
        msg << actual_data << ", but should have produced " << data.value()
            << " bytes";
        throw TCPExpectationViolation(msg.str());
      }
    }
...
struct ExpectSegment : public TCPExpectation {
  ...
  virtual TCPSegment expect_seg(TCPTestHarness &harness) const {
    ...
    if (seg.length_in_sequence_space() > TCPConfig::MAX_PAYLOAD_SIZE) {
      throw SegmentExpectationViolation(
          "packet has length_including_flags (" +
...
struct ExpectUnassembledBytes : public TCPExpectation {
  ...
  void execute(TCPTestHarness &harness) const {
    ...
    if (actual_unassembled_bytes != bytes) {
      throw TCPPropertyViolation::make("unassembled_bytes", bytes,
...
struct ExpectLingerTimer : public TCPExpectation {
  ...
  void execute(TCPTestHarness &harness) const {
    ...
    if (actual_ms != ms) {
      throw TCPPropertyViolation::make("time_since_last_segment_received", ms,
                                       actual_ms);
    }
  }
};
...
struct SendSegment : public TCPAction {
  ...
  SendSegment &with_seqno(uint32_t seqno_) {
    return with_seqno(WrappingInt32{seqno_});
  }
  ...
  SendSegment &with_ackno(uint32_t ackno_) {
    return with_ackno(WrappingInt32{ackno_});
  }
...
struct Write : public TCPAction {
  ...
  void execute(TCPTestHarness &harness) const {
    ...
    if (_bytes_written.has_value() and
        bytes_written != _bytes_written.value()) {
