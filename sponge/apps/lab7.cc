  ...
  case EthernetHeader::TYPE_IPv4: {
    ...
    if (dgram.parse(frame.payload().concatenate()) == ParseResult::NoError) {
      ...
      if (dgram.header().proto == IPv4Header::PROTO_TCP) {
        TCPSegment tcp_seg;
        if (tcp_seg.parse(dgram.payload().concatenate(),
...
private:
  ...
  void send_pending() {
    while (not _interface.frames_out().empty()) {
      _data_socket_pair.first.write(
          _interface.frames_out().front().serialize());
...
public:
  TCPSocketLab7(const Address &ip_address, const Address &next_hop)
      : TCPSpongeSocket<NetworkInterfaceAdapter>(
  ...
  void listen_and_accept() {
    FdAdapterConfig multiplexer_config;
    multiplexer_config.source = _local_address;
    TCPSpongeSocket<NetworkInterfaceAdapter>::listen_and_accept(
        {}, multiplexer_config);
  }
...
void program_body(bool is_client, const string &bounce_host,
  ...
  if (is_client) {
    ...
    internet_side = router.add_interface(
        {random_router_ethernet_address(), {"10.0.0.192"}});
    ...
    internet_side = router.add_interface(
        {random_router_ethernet_address(), {"10.0.0.172"}});
  ...
  thread network_thread([&]() {
    try {
      ...
      event_loop.add_rule(sock.adapter().frame_fd(), Direction::In, [&] {
        EthernetFrame frame;
        if (frame.parse(sock.adapter().frame_fd().read()) !=
            ParseResult::NoError) {
          return;
        }
        if (debug) {
      ...
      event_loop.add_rule(
          sock.adapter().frame_fd(), Direction::Out,
          [&] {
