...
static int process_ipv4_ipv6(int len, const uint8_t *data, string &src_addr,
                             string &dst_addr) {
...
int main(int argc, char **argv) {
  ...
  const int dl_type = [&] {
    ...
    if (p_hdl == nullptr) {
      cout << "\nError initiating capture: " << static_cast<char *>(errbuf)
           << endl;
      exit(1);
    }
    int dlt = pcap_datalink(p_hdl);
    // need to handle: DLT_RAW, DLT_NULL, DLT_EN10MB, DLT_LINUX_SLL
    if (dlt != DLT_RAW && dlt != DLT_NULL && dlt != DLT_EN10MB &&
        dlt != DLT_LINUX_SLL
#ifdef DLT_LINUX_SLL2
        && dlt != DLT_LINUX_SLL2
#endif
    ) {
      cout << "\nError: unsupported datalink type "
           << pcap_datalink_val_to_description(dlt) << endl;
      exit(1);
    }
  ...
  // compile and set filter
  {
    ...
    if (pcap_compile(p_hdl, &p_flt, filter_expression.c_str(), 1,
                     PCAP_NETMASK_UNKNOWN) != 0) {
      cout << "Error compiling filter expression: " << pcap_geterr(p_hdl)
           << endl;
      return EXIT_FAILURE;
    }
  ...
  while ((next_ret = pcap_next_ex(p_hdl, &pkt_hdr, &pkt_data)) >= 0) {
    ...
    string src{}, dst{};
    if ((start_off = process_ipv4_ipv6(pkt_hdr->caplen - hdr_off,
                                       pkt_data + hdr_off, src, dst)) < 0) {
    ...
    string_view payload{reinterpret_cast<const char *>(pkt_data) + payload_off,
                        payload_len};

    // try to parse UDP payload as TCP packet
    auto seg = TCPSegment{};
    if (const auto res = seg.parse(string(payload), 0);
        res > ParseResult::BadChecksum) {
      ...
      cout << src << ':' << tcp_hdr.sport << " > " << dst << ':'
           << tcp_hdr.dport << "\n    Flags ["

           << (tcp_hdr.urg ? "U" : "") << (tcp_hdr.psh ? "P" : "")
           << (tcp_hdr.rst ? "R" : "") << (tcp_hdr.syn ? "S" : "")
           << (tcp_hdr.fin ? "F" : "") << (tcp_hdr.ack ? "." : "")
      ...
      cout << " ack " << tcp_hdr.ackno << " win " << tcp_hdr.win << " length "
           << payload_len << endl;
    }
