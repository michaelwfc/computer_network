...
static void show_usage(const char *argv0, const char *msg) {
  cout << "Usage: " << argv0 << " [options] <host> <port>\n\n"
       ...
       << LOCAL_ADDRESS_DFLT << "\n"
       << "   -s <port>       Set TCP source port (client mode only)          "
          "(random)\n\n"
...
get_config(int argc, char **argv) {
  ...
  while (argc - curr > 2) {
    if (strncmp("-a", argv[curr], 3) == 0) {
      ...
      show_usage(
          argv[0],
          string("ERROR: unrecognized option " + string(argv[curr])).c_str());
      exit(1);
    }
  }
...
int main(int argc, char **argv) {
  try {
    ...
    // choose a random local Ethernet address (and make sure it's private, i.e.
    ...
    TCPOverIPv4OverEthernetSpongeSocket tcp_socket(
        TCPOverIPv4OverEthernetAdapter(TCPOverIPv4OverEthernetAdapter(
            TapFD(tap_dev_name), local_ethernet_address, c_filt.source,
            next_hop)));
