...
void bidirectional_stream_copy(Socket &socket) {
  ...
  _eventloop.add_rule(
      ...
      [&] {
        return (not _outbound.error()) and
               (_outbound.remaining_capacity() > 0) and (not _inbound.error());
      },
      [&] { _outbound.end_input(); });
  ...
  _eventloop.add_rule(
      ...
      [&] {
        ...
        const size_t bytes_written =
            socket.write(_outbound.peek_output(bytes_to_write), false);
      ...
      [&] {
        return (not _outbound.buffer_empty()) or
               (_outbound.eof() and not _outbound_shutdown);
      },
      [&] { _outbound.end_input(); });
  ...
  _eventloop.add_rule(
      ...
      [&] {
        return (not _inbound.error()) and
               (_inbound.remaining_capacity() > 0) and (not _outbound.error());
      },
      [&] { _inbound.end_input(); });

  // rule 4: read from inbound byte stream into stdout
  _eventloop.add_rule(
      _output, Direction::Out,
      [&] {
        const size_t bytes_to_write =
            min(max_copy_length, _inbound.buffer_size());
        const size_t bytes_written =
            _output.write(_inbound.peek_output(bytes_to_write), false);
      ...
      [&] {
        return (not _inbound.buffer_empty()) or
               (_inbound.eof() and not _inbound_shutdown);
      },
      [&] { _inbound.end_input(); });
